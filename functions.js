...
global.OnRegistered = function (pData) {
    ...
    for(var i = 0; i < pData.user.length; ++i) {
        ...
        HandleBans(sUser);
       /* if(mBanList[sUser.userid] != undefined && mBanList[sUser.userid] != null
            && mBanList[sUser.userid])
        {
          mUsers[sUser.userid].Boot("We prefer to not have your company, thanks.");
        }*/
        if(sUser.name.indexOf("ttstats") != -1) mUsers[sUser.userid].Boot("No ttstats bots, thanks.");
        //if(sUser.userid == '4e3fe846a3f7512f190249da') HandleCommand(sUser, "/slap @" + sUser.name);
        //if(sUser.userid == '4e4d05b1a3f7510459174bc5')
        //{
          //mBot.speak("trying: /slap @" + sUser.name);
          //HandleCommand(sUser, '/slap @' + sUser.name, false);
        //}
    }
...
global.HandleBans = function(pUser){
    var sBannedUsers = mBanList.filter(function(pBan){
                return pBan.userid == pUser.userid;
        });
    sBannedUsers.forEach(function (pBan) {
                if(pBan.banned){
                  mUsers[pBan.userid].Boot("We prefer to not have your company, thanks.");
                }
        });    
};
...
global.OnBoot = function (pData) {
/*
{ command: 'booted_user',
  userid: '123456789012345',
  reason: 'You were bad.',
  modid: '123456789012345',
  roomid: '123456789012345', 
  success: true }
*/
  
  if(pData['modid'] && pData['modid'] == '4e0b37d5a3f75146740429f9')
  {//Beez
    mBot.speak('http://i.imgur.com/xDj9r.jpg');
  }
  else
  {
...
global.ObjectToString = function(pData) {
  var objectString = "";
  
  Log("about to iterate over pData properties");
  for(var propertyName in pData)
  {
    Log("found property named: " + propertyName);
    propertyVal = pData[propertyName];
    propertyValAsString = "";
    if(propertyVal instanceof Array)
    {
      Log("property " + propertyName + " is an array.");
      propertyValAsString = "[";
      for(i=0; i<propertyVal.length; i++)
      {
        //propertyValAsString += ObjectToString(propertyVal[i]);
        propertyValAsString += propertyVal[i];
      }
      propertyValAsString += "]"
    }
    else
    {
      Log("property " + propertyName + " has a value of " + propertyVal);
      //propertyValAsString = ObjectToString(propertyVal);
    }
    
    if(propertyValAsString != "")
    {
      if(objectString == null) objectString = "";
      objectString += "\t" + propertyName + ": " + pData[propertyName] + ",\n";
    }
  }
  Log("done iterating over pData properties");
...
global.GreetDJ = function(pUser){

  try
  {
    if(DJImages[pUser.userid])
    {
      Speak(pUser, DJImages[pUser.userid].img, SpeakingLevel.DJChange);
    }
  }
  catch(err)
  {
...
//  if(pUser.userid == '50030e8deb35c12c6a0002aa')
//  {
//    mUsers[sUser.userid].Boot('We don''t take kindly to lamers in here.');
//  }
//  else
//  {
    if(pUser.customDJGreeting)
    {
      Speak(pUser, pUser.customDJGreeting, SpeakingLevel.DJChange);
    }
    else
    {
      Speak(pUser, mAddDJ, SpeakingLevel.DJChange);  
    }
//  }
};
...
//    if(mCurrentDJ && !mCurrentDJ.isVip) {
    ...
    if((mCurrentDJ.GetLevel() > 2 && mAutoBopForMods) || mCurrentDJ.autoBopFor) setTimeout(function () {
        mBot.vote("up");
    }, 5000);
};

global.HandleAutoTweet = function(pData) {
//  Log('artist: ' + pData.room.metadata.current_song.metadata.artist);
  var sArtistTwitter = mArtistTwitters[pData.room.metadata.current_song.metadata.artist];
  
  if(sArtistTwitter != undefined && 
      sArtistTwitter != null)
  {
//    Log('OK...I found the artist\'s twitter object');
//    Log('autotweet? ' + sArtistTwitter.autotweet);
    if(sArtistTwitter.autotweet)
    {
//      Log('OK...calling command to tweet');
      var sUser = mUsers[pData.room.metadata.current_dj];
      HandleCommand(sUser, '/tweet' ,false);
    }
  }
};

global.HandleArtist = function(pData) {
	var sUser = mUsers[pData.room.metadata.current_dj];
  var sCurrentSong = pData.room.metadata.current_song.metadata;
        sArtist = mArtists.filter(function(pArtist){
                //mBot.speak("pArtist.artist: " + pArtist.artist);
                //mBot.speak("artist: " + pData.room.metadata.current_song.metadata.artist);
                //mBot.speak("indexOf: " + pData.room.metadata.current_song.metadata.artist.indexOf(pArtist));
                
                return pData.room.metadata.current_song.metadata.artist.indexOf(pArtist.artist) >= 0;
        });
        //mBot.speak("sArtist" + sArtist);
        if(sArtist)
        {
                sArtist.forEach(function (pArtist) {
                        pArtist.callback(sUser, sCurrentSong);
                });
        }
};
...
global.OnSpeak = function (pData) {
    ...
    HandleTWSS(sUser, sText);
    CheckAutoBan(sUser, sText);
};

global.HandleTWSS = function(pUser, pText) {
  if(pText == 'TWSS')
  {
    try
    {
      HandleCommand(pUser, "/react twss");
    }
    catch(err)
    {
      Log("Crap. HandleCommand(pUser, '/react twss'); took a dump on me: " + err);
    }
...
global.OnVote = function (pData) {
    ...
    if(mAfkBop) {
        ...
        try
        {
          mBot.roomInfo(function (data) {
                          var sUsers = data.users.filter(function (pUser) {
                                            return pUser.userid == data.room.metadata.current_dj;
                                        });
                          sUsers.forEach(function (lUser) {
                                          points = lUser.points * 1;
                                          //Log(points);
                                          //mBot.speak("I think you have " + points + "DJ points");
                                          if((points%1000) == 0)
                                          {
                                            mBot.speak("Congrats on " + points + " points!");
                                            HandleCommand(mUsers[lUser.userid], "/react celebrate");
                                          }
...
global.InitMySQLDB = function() {
  Log("Initializing MySQL DB Connection");
  var connectionProperties = {
                              host: global.mMySQLHost,
                              user: global.mMySQLUser,
                              password: global.mMySQLPass,
                              port: global.mMySQLPort,
                              db: global.mMySQLDatabase
                              };
  global.mMySQLDB = mMySQL.createConnection(connectionProperties);
